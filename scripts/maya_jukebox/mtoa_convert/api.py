...
def publish_objs(objs_dict, assets_dir, refs_dir, textures_dir):
    ...
    for asset_name, obj_path in objs_dict.items():
        ...
        asset_locator = cmds.spaceLocator(
            name="{}_loc".format(asset_name), position=(0, 0, 0)
        )
...
def convert_materials_to_arnold(textures_dir):
    for material, original_shading_group in utils.get_scene_materials():
        ...
        for component, (map, colourspace) in constants.texture_mappings.items():
            ...
            file_node = cmds.rename(
                file_node, os.path.splitext(os.path.basename(map_filepath))[0]
            )
            cmds.setAttr(
            ...
            alpha_flag = (
                component in constants.alpha_invert
            ...
            if component == "normalCamera":
                ...
                cmds.connectAttr(
                    ...
                    "{}.input".format(range_node),
                    force=True,
                )
                cmds.connectAttr(
                    "{}.outColorR".format(range_node),
                    "{}.{}".format(shader, component),
                    force=True,
                )
            else:
...
def map_interiors(interior_mapping_json=None):
    ...
    interiors_ma = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "interior_shaders.ma"
    )
    ...
    for interior_name_pattern in constants.interior_naming:
        for interior_transform in cmds.ls(interior_name_pattern, transforms=True):
            ...
            if (
                "_exposition_" not in interior_transform
                and "hop_interior_" not in interior_transform
            ):
                clean_interior_transform = ("_").join(
                    interior_transform.split("_")[0:-1]
                )
            ...
            if interior_mapping.has_key(clean_interior_transform):
                ...
                print(
                    "Assigning {} to transform {}".format(
                        shading_group, interior_transform
                    )
                )
                cmds.sets(
                    interior_transform, e=1, forceElement=shading_group, noWarnings=True
                )
