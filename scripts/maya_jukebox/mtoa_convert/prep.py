...
def export_interior_mappings(
    fbx_source_folder="C:\\Users\\their\\Documents\\AndreJukebox\\fbx_refs",
    ...
    for fbx_path in fbx_paths:
        ...
        for interior_name_pattern in constants.interior_naming:
            for transform in cmds.ls(interior_name_pattern, transforms=True):
                shader_engine = cmds.listConnections(
                    cmds.listHistory(transform), type="shadingEngine"
                )
                shaders = cmds.ls(
                    cmds.listConnections(shader_engine), materials=True, type="phong"
                )
                if shaders:
                    shader = shaders[0]
                    if not shader.startswith(("am215", "AM215")):
                        continue
                    if (
                        interior_name_pattern == constants.interior_naming[-1]
                    ):  #  "*hop_interior_*" only has one shader
                        transform = ("_").join(transform.split("_")[0:-1])
                        print(transform)
                        version = 1
                    elif (
                        interior_name_pattern == constants.interior_naming[0]
                        and constants.interior_naming[-1] not in transform
                    ):
                        transform = ("_").join(transform.split("_")[0:-1])
                        print(transform)
                    ...
                    interior_mapping[transform.lower()] = constants.interior_mapping[
                        interior_name_pattern
                    ].format(version)
