...
def export_interior_mappings(fbx_source_folder="C:\\Users\\their\\Documents\\AndreJukebox_Irene\\fbx_refs",
    ...
    for fbx_path in fbx_paths:
        ...
        ##############################################################        
        for transform in cmds.ls(transforms=True):
            shader_engines = cmds.listConnections(cmds.listHistory(transform), type = 'shadingEngine')
            if shader_engines:
                shader_engines = list(set(shader_engines))                
                if len(shader_engines) > 1:
                    if multiple_shaders.has_key(building):
                        multiple_shaders[building].update({transform: shader_engines})
                    else:
                        multiple_shaders[building] = {transform: shader_engines}
        multiple_shaders_outfile = "C:\\Users\\their\\Documents\\AndreJukebox_Irene\\multiple_shaders_unique.json" 
        with open(multiple_shaders_outfile, "w") as multiple_shaders_outfile:
            json.dump(multiple_shaders, multiple_shaders_outfile)
        #################################################################
        ...
        #         print(shader_engine)
        #         if len(shader_engine) > 1:
        #             if multiple_shaders.has_key(building):
        #                 multiple_shaders[building].update({transform: shader_engine})
        #             else:
        #                 multiple_shaders[building] = {transform: shader_engine}
    ...
    # unmapped_outfile = "C:\\Users\\their\\Documents\\AndreJukebox_Irene\\unmapped_shaders.json" 
