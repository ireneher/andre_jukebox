...
class QStyleSheet(UserString):
    ...
    def __init__(self, cls=None, name=None, psuedo=None, **styles):
        ...
        self._styles = (
            OrderedDict()
        )  # we'll preserve the order of attributes given - python 3.6+
        if styles:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.LABEL_STYLE = QStyleSheet(
            QLabel, color=self.DARK_BLUE, font_weight=400, font_size="9pt"
        )
        ...
        s1.setStyles(
            cls=QPushButton, psuedo=":pressed", background_color=self.PALE_SALMON
        )

        s1.setStyles(
            cls=QPushButton, psuedo=":focus-pressed", background_color=self.PALE_SALMON
        )

        s1.setStyles(
            cls=QPushButton, psuedo=":disabled", background_color=self.LIGHT_GREY
        )

        s1.setStyles(
            cls=QPushButton, psuedo=":checked", background_color=self.PALE_SALMON
        )

        s1.setStyles(
            cls=QPushButton,
            psuedo=":hover:!pressed:!checked",
            background_color=self.MED_BLUE,
        )
        self.BUTTON_STYLE = str(self.BUTTON_STYLE)

        self.BUTTON_GROUPBOX_STYLE = QStyleSheet(
            QGroupBox, border="none", font_weight="bold", color="white"
        )
        self.BUTTON_GROUPBOX_STYLE = str(self.BUTTON_GROUPBOX_STYLE)

        self.TEXT_EDIT_STYLE = QStyleSheet(
            QTextEdit,
            color="white",
            border=f"1px solid {self.LIGHT_BLUE}",
            background_color=self.MED_BLUE,
        )
    ...
    def initUI(self):
        ...
        collapsible.setToggleButtonStyles(
