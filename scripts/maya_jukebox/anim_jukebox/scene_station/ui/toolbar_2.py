...
def mode_action(
    ...
    checkable=True,
    checked=True,
    icon_path=os.path.join(ICON_DIRECTORY, HAND_ICON_FILENAME),
):
    return create_action(parent, callback, text, checkable, checked, icon_path)


def add_graph_action(
    parent,
    callback=(lambda event: print("Add Graph Clicked!")),
    text="Add Graph",
    checkable=False,
    checked=False,
    icon_path=os.path.join(ICON_DIRECTORY, ADD_GRAPH_ICON_FILENAME),
):
    return create_action(parent, callback, text, checkable, checked, icon_path)


def delete_graph_action(
    parent,
    callback=(lambda event: print("Delete Graph Clicked!")),
    text="Delete Graph",
    checkable=False,
    checked=False,
    icon_path=os.path.join(ICON_DIRECTORY, DELETE_GRAPH_ICON_FILENAME),
):
    return create_action(parent, callback, text, checkable, checked, icon_path)
...
def list_action(
    ...
    checkable=False,
    checked=False,
    icon_path=os.path.join(ICON_DIRECTORY, LIST_ICON_FILENAME),
):
    return create_action(parent, callback, text, checkable, checked, icon_path)
